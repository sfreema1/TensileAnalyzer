...
results(1,:) = {'Name','Width (mm)','Thickness (mm)','Elastic Modulus (Pa)',...
    'UTS (Pa)','Burst pressure (mmHg)','SEM of measurement','Toughness (J/m^3)'};
%% Analyzing graphs
for i = 1:n
    ...
    width = data_cell{i}.width; % in mm
    thickness = data_cell{i}.thickness; % in um
    OD = data_cell{i}.OD; % in mm
    ...
    stress = smoothforce(cv_index:cv_index2)/(thickness*width/(10^3)/(10^6));
